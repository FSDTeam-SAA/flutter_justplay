...
class AuthController extends BaseController {
  ...
  Future<void> login(String name, String phone, BuildContext context) async {
    ...
    result.fold(
        ...
        await Get.find<RealtimeController>().connect(user.id);
        unawaited(Get.find<PushNotificationService>().initialize());
  ...
  Future<void> logout(BuildContext context) async {
    await Get.find<PushNotificationService>().unregisterCurrentToken();
    await _authStorageService.clearAuthData();
    await _secureStoreServices.deleteData(KeyConstants.conversationId);
    Get.find<RealtimeController>().disconnect();
